@testset "1022.sum-of-root-to-leaf-binary-numbers.jl" begin
    ...
    ## BFS Algorithm
    @test sum_root_to_leaf_bfs(TreeNode{Int}([1, 0, 1, 0, 1, 0, 1])) == 22
    @test sum_root_to_leaf_bfs(TreeNode(1)) == 1
    @test sum_root_to_leaf_bfs(TreeNode(0)) == 0
    @test sum_root_to_leaf_bfs(TreeNode{Int}([1, 1])) == 3
end
